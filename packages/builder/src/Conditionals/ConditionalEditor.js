...
export const ConditionalEditor = ({ condition, onChange, fields, magics }) => {
    ...
    function randomString(prefix) {
        ...
        return prefix + [getRandomInt(10), getRandomInt(getRandomInt(10)), getRandomInt(10),getRandomInt(10),getRandomInt(10)].join('');
    }
    ...
    const topProps = { onChangeName, onChangeType, id, type, name, addRule:onAddGroup  };
		...
		<div className="caldera-editor-condition-config caldera-forms-condition-edit">
			...
			{groupRulesIds.map(groupId => {
                ...
                groupRulesIds.indexOf(groupId) === groupRulesIds.length - 1
						? true
                        : false;
                const ruleProps = {  onChange: group => {
                    onChange(
                        setGroup(groupId,group)
                    );   
                }, fields, magics,isLast };
				return (
