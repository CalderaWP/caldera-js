...
describe("updateRows", () => {
	...
	it("should remove a hidden row", () => {
		...
		expect(
			updateRows(state, rows)[0].columns[0].fields.findIndex(
				f => f.fieldId === "showField"
			)
		).toEqual(0);
		expect(
			updateRows(state, rows)[0].columns[0].fields.findIndex(
				f => f.fieldId === "hideField"
			)
		).toEqual(-1);
	});
	...
	it("should replace field ids with fields", () => {
		...
		expect(
			updateRows(state, rows, [field])[0].columns[0].fields.find(
				f => f.fieldId === "showField"
			)
		).toEqual(field);
	});
	...
	it("should remove invalid fields", () => {
		...
		expect(
			updateRows(state, rows, [field])[0].columns[0].fields.length
		).toEqual(1);
	});
	...
	it("should allow rows to have render", () => {
		...
		expect(typeof updateRows(state, rows, [field])[0].render).toEqual(
			"function"
		);
	});
	...
	it("should allow columns to have render", () => {
		...
		expect(
			typeof updateRows(state, rows, [field])[0].columns[0].render
		).toEqual("function");
	});
	...
	it("should allow fields to have render", () => {
		...
		expect(
			typeof updateRows(state, rows, [field])[0].columns[0].fields[0]
				.render
		).toEqual("function");
	});
});
